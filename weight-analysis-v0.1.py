...
decoder_keys = [k for k in state_dict.keys() if "decoder" in k and ("weight" in k or "bias" in k)]
...
if len(decoder_keys) >= 3:
    target_keys = [
        decoder_keys[int(len(decoder_keys) * 0.1)],  # Truly Early (e.g., lower block numbers)
        decoder_keys[int(len(decoder_keys) * 0.5)],  # Truly Mid
        decoder_keys[int(len(decoder_keys) * 0.9)]   # Truly Late (e.g., higher block numbers)
    ]
else:
...
for depth_idx, param_key in enumerate(target_keys):
    ...
    for f in scaling_factors:
        ...
        means.append(torch.mean(absdiff).item())
        errors.append(torch.std(absdiff).item())
        losses.append(mrstft(output_tensor.unsqueeze(0), output_clean.unsqueeze(0)).item())
        
        # Save audio files
        if f in [scaling_factors[0], scaling_factors[-1]]:
            f_str = f"{f:.2f}".replace('.', '_')
            out_name = reconstructed_path / f"{Path(file_name).stem}_depth{depth_idx+1}_drastic_factor{f_str}.wav"
            torchaudio.save(out_name, output_tensor, sr)
