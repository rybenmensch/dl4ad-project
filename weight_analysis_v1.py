...
def get_root_module_path(module_path: str) -> str | None:
    # these root names should be in some library or something
    root_names: Tuple[str, str] = ("encoder", "decoder")
    for name in root_names:
        if module_path.startswith(name):
            return name
...
# rave
def get_weight_suffixes() -> Tuple[str, str, str, str]:
    return (
        "weight",
        "weight_g",
        "weight_v",
...
def parameter_key_to_module_path(parameter_key: str) -> str | None:
    ...
    # basically just strips the 'weight' extension
    # maybe not structured well because it both checks and returns a result

    for suffix in get_weight_suffixes():
        s = "." + suffix
        if parameter_key.endswith(s):
            return parameter_key[: -len(s)]

    warnings.warn(f"{parameter_key=} has an unsupported suffix!")
    return None


def collect_weight_module_paths(model: torch.nn.Module, section_name: str) -> list[str]:
    """
    Collect valid weight-bearing modules in one model section from state_dict.
    """
    module_paths = set()
    ...
    for parameter_key in model.state_dict().keys():
        ...
        # parameter_key: section_name.sub...last.weights_suffix
        # module_path:   section_name.sub...last
    ...
    # kinda defensive, I think we already know whether or not we have weights
    # valid_paths = []
    #
    # for module_path in sorted_paths:
    #     try:
    #         module = get_submodule(model, module_path)
    #         has_weight = any(hasattr(module, ext) for ext in get_weight_suffixes())
    #         if has_weight:
...
def select_representative_depths(
    ...
    # TODO: do this thing but by going over the indices of the subnet
    # for p in module_paths:
    #     get_root_module_path(p)

    # this is kinda dumb, the layers literally have indices.
...
def extract_audio_features(
    ...
    # we should just do this with a library
    # or just put it into a library once again
...
torchaudio.save(str(clean_output_path), output_clean, sample_rate)
...
for network_section, depth_name, module_path in experiments:
    ...
    for factor in scaling_factors:
        ...
        factor_is_saved: bool = any(np.isclose(factor, s_v) for s_v in save_factors)
